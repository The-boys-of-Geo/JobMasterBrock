...
//****** Scrape Data ******//
scraperController.scrapeData = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    const { search, location, time } = req.body;
    ...
    let url = `https://www.linkedin.com/jobs/search?keywords=${searchParsed}&location=${locationParsed}&sortBy=R&f_TPR=r${timeParsed}&f_JT=${jobTypeParsed}&position=1&pageNum=0`;
    ...
    const err = {
      ...
      status: 404,
      message: {
        err: 'Error in scraperController.scrapeData: Check server log for more details.',
      },
    };
    return next(err);
  }
